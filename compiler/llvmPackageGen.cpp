...
BIRPackage::BIRPackage(string orgName, string pkgName, string verName, string srcFileName)
...
void BIRPackage::translate(LLVMModuleRef &modRef) {
    ...
    GlobalVariable *charGloablVar = new GlobalVariable(*unwrap(modRef), unwrap(charPtrType), false,
                                                       GlobalValue::InternalLinkage, charValue, strTablePtr, 0);
    ...
    for (auto const it : globalVars) {
        ...
        LLVMTypeRef varTyperef = funcObj->getLLVMTypeRefOfType(globVar->getTypeDecl());
        ...
        if (varTyperef && modRef) {
            ...
            GlobalVariable *gVar = new GlobalVariable(*unwrap(modRef), unwrap(varTyperef), false,
    ...
    for (it = functionLookUp.begin(); it != functionLookUp.end(); it++) {
        ...
        for (unsigned i = 0; i < numParams; i++) {
            ...
            if (funcParam && funcParam->typeTag() == TYPE_TAG_ANY) {
                ...
                paramTypes[i] = birFunc->getLLVMTypeRefOfType(funcParam->getVarDecl()->getTypeDecl());
            }
        }
        if (retType) {
            funcType = LLVMFunctionType(retType, paramTypes, numParams, isVarArg);
            if (funcType) {
                birFunc->setNewFunctionRef(LLVMAddFunction(modRef, birFunc->getName().c_str(), funcType));
            }
        }
    }
    ...
    if (strBuilder->getSize() != 0) {
        ...
        LLVMValueRef bitCastRes = LLVMBuildBitCast(builder, stringTableAddr, LLVMPointerType(LLVMInt8Type(), 0), "");
        LLVMSetInitializer(stringTablePtrAddress, bitCastRes);
    }
}

void BIRPackage::addStringOffsetRelocationEntry(string eleType, LLVMValueRef storeInsn) {
    if (structElementStoreInst.size() == 0) {
        vector<LLVMValueRef> temp;
        temp.push_back(storeInsn);
        structElementStoreInst.insert(pair<string, vector<LLVMValueRef>>(eleType, temp));
    } else {
        ...
        if (itr == structElementStoreInst.end()) {
            ...
            structElementStoreInst.insert(pair<string, vector<LLVMValueRef>>(eleType, temp1));
        } else {
...
void BIRPackage::applyStringOffsetRelocations(LLVMModuleRef &modRef) {
    ...
    for (itr = structElementStoreInst.begin(); itr != structElementStoreInst.end(); itr++) {
        ...
        for (unsigned int i = 0; i < temp.size(); i++) {
            ...
            GetElementPtrInst *GEPInst = dyn_cast<GetElementPtrInst>(unwrap(storeInsn));
