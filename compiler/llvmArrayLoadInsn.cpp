...
ArrayLoadInsn::ArrayLoadInsn(Location *pos, InstructionKind kind, Operand *lOp, bool opFA, bool fR, Operand *KOp,
                             Operand *ROp)
    : NonTerminatorInsn(pos, kind, lOp), optionalFieldAccess(opFA), fillingRead(fR), keyOp(KOp), rhsOp(ROp) {}

LLVMValueRef ArrayLoadInsn::getArrayLoadDeclaration(LLVMModuleRef &modRef, BIRPackage *pkg) {
    ...
    LLVMValueRef addedFuncRef = LLVMAddFunction(modRef, "int_array_load", funcType);
...
void ArrayLoadInsn::translate(LLVMModuleRef &modRef) {
    ...
    LLVMValueRef ArrayLoadFunc = pkgObj->getFunctionRefBasedOnName("int_array_load");
    if (!ArrayLoadFunc)
