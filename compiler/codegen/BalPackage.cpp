...
void Package::translate(LLVMModuleRef &modRef) {
  ...
  for (const auto &function : functions) {
    ...
    LLVMTypeRef funcType = LLVMFunctionType(function->getLLVMTypeOfReturnVal(),
                                            paramTypes, numParams, isVarArg);
    if (funcType) {
      function->setLLVMFunctionValue(
          LLVMAddFunction(modRef, function->getName().c_str(), funcType));
    }
  }
