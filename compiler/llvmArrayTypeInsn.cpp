...
ArrayInsn::ArrayInsn(Location *pos, InstructionKind kind, Operand *lOp, Operand *sOp, TypeDecl *tDecl)
    : NonTerminatorInsn(pos, kind, lOp), sizeOp(sOp), typeDecl(tDecl) {}

LLVMValueRef ArrayInsn::getNewArrayDeclaration(LLVMModuleRef &modRef, BIRPackage *pkg) {
    ...
    LLVMValueRef addedFuncRef = LLVMAddFunction(modRef, "new_int_array", funcType);
...
void ArrayInsn::translate(LLVMModuleRef &modRef) {
    ...
    LLVMValueRef newArrayFunc = pkgObj->getFunctionRefBasedOnName("new_int_array");
    if (!newArrayFunc)
        newArrayFunc = getNewArrayDeclaration(modRef, pkgObj);
    assert(sizeOpValueRef && newArrayFunc);
    LLVMValueRef newArrayRef = LLVMBuildCall(builder, newArrayFunc, sizeOpValueRef, 1, "");
